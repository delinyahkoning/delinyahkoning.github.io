(function(){
  ...
  if (!lb || !frame || !closeBtn || !slot) return;
  ...
  function openWithMedia(mediaEl){
    ...
    if (isVideo){
      ...
      const startAndPlay = () => { try{ clone.currentTime = 0; }catch{} const p = clone.play(); if (p?.then) p.catch(()=>{}); };
      clone.addEventListener('loadedmetadata', startAndPlay, { once:true });
      requestAnimationFrame(startAndPlay);
    }
    slot.appendChild(clone);
    lb.classList.add('open'); lb.setAttribute('aria-hidden','false'); closeBtn.focus();
  }

  function close(){ lb.classList.remove('open'); lb.setAttribute('aria-hidden','true'); slot.innerHTML=''; }

  function bind(scope=document){
    scope.querySelectorAll('.thumb[data-lightbox]').forEach(thumb=>{
  ...
  // when cards are (re)built by config
  document.addEventListener('cards:hydrated', ()=>bind(document));
})();
